...
class PDFReportGenerator:
    ...
    def _add_title_page(self):
        ...
        self.pdf.cell(0, 40, "Research Report", 0, 1, 'C')
        self.pdf.set_font('DejaVu', '', 14)
        self.pdf.cell(0, 10, "Generated by Advanced Research Assistant", 0, 1, 'C')
        self.pdf.ln(30)
        self.pdf.set_font('DejaVu', 'I', 10)
        self.pdf.cell(0, 10, f"Generated on: {datetime.now().strftime('%B %d, %Y at %H:%M')}", 0, 1, 'C')
        self.pdf.add_page()
    ...
    def _add_heading(self, text: str, level: int):
        ...
        if level == 1:
            ...
            self.pdf.ln(8)
            self.pdf.cell(0, 8, text, 0, 1)
            self.pdf.ln(4)
        else:  # level 3
            self.pdf.set_font('DejaVu', 'B', 12)
            self.pdf.ln(6)
            self.pdf.cell(0, 6, text, 0, 1)
            self.pdf.ln(3)
    ...
    def _add_footer(self):
        """Add footer to each page"""
        # Store current auto page break setting
        auto_page_break = self.pdf.auto_page_break
        self.pdf.auto_page_break = False
        
        # Iterate through all pages
        for i in range(1, self.pdf.page_no() + 1):
            self.pdf.page = i  # Switch to page
            self.pdf.set_y(-15)
            self.pdf.set_font('DejaVu', 'I', 8)
            self.pdf.cell(0, 10, f"Page {i} of {self.pdf.page_no()}", 0, 0, 'C')
        
        # Restore auto page break setting
        self.pdf.auto_page_break = auto_page_break